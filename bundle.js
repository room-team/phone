...
var asteroid = new Asteroid({ endpoint: 'wss://roomteam.me:443/websocket' });

asteroid.ddp.on('socket_close', function(){
	console.log("error");
});
...
window.getEmptyRooms = function (callback) {
...
window.setRoom = function (roomId, callback) {
    if (currentLogin != null) {
        asteroid.call("accounts.setRoom", roomId).then(function (result) {
            console.log(result);
        }).catch(function (error) {
            console.log("Error");
            console.error(error);
